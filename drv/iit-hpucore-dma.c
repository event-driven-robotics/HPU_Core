...
static struct debugfs_reg32 hpu_regs[] = {
	{"HPU_CTRL_REG",                   HPU_CTRL_REG},
	{"HPU_LPBK_LR_CNFG_REG",           HPU_LPBK_LR_CNFG_REG},
	{"HPU_RXDATA_REG",                 HPU_RXDATA_REG},
	{"HPU_RXTIME_REG",                 HPU_RXTIME_REG},
	{"HPU_DMA_REG",                    HPU_DMA_REG},
	{"HPU_STAT_RAW_REG",               HPU_STAT_RAW_REG},
	{"HPU_IRQ_REG",                    HPU_IRQ_REG},
	{"HPU_MSK_REG",                    HPU_MSK_REG},
	{"HPU_WRAPTIMESTAMP_REG",          HPU_WRAPTIMESTAMP_REG},
	{"HPU_HSSAER_STAT_REG",            HPU_HSSAER_STAT_REG},
	{"HPU_HSSAER_RX_ERR_REG",          HPU_HSSAER_RX_ERR_REG},
	{"HPU_HSSAER_RX_MSK_REG",          HPU_HSSAER_RX_MSK_REG},
	{"HPU_RX_CTRL_REG",                HPU_RX_CTRL_REG},
	{"HPU_TX_CTRL_REG",                HPU_TX_CTRL_REG},
	{"HPU_RX_PAER_CNFG_REG",           HPU_RX_PAER_CNFG_REG},
	{"HPU_TX_PAER_CNFG_REG",           HPU_TX_PAER_CNFG_REG},
	{"HPU_IP_CNFG_REG",                HPU_IP_CNFG_REG},
	{"HPU_FIFO_THRSH_REG",             HPU_FIFO_THRSH_REG},
	{"HPU_LPBK_AUX_CNFG_REG",          HPU_LPBK_AUX_CNFG_REG},
	{"HPU_ID_REG",                     HPU_ID_REG},
	{"HPU_AUX_RX_CTRL_REG",            HPU_AUX_RX_CTRL_REG},
	{"HPU_HSSAER_AUX_RX_ERR_REG",      HPU_HSSAER_AUX_RX_ERR_REG},
	{"HPU_HSSAER_AUX_RX_MSK_REG",      HPU_HSSAER_AUX_RX_MSK_REG},
	{"HPU_HSSAER_AUX_RX_ERR_THR_REG",  HPU_HSSAER_AUX_RX_ERR_THR_REG},
	{"HPU_HSSAER_AUX_RX_ERR_CH0_REG",  HPU_HSSAER_AUX_RX_ERR_CH0_REG},
	{"HPU_HSSAER_AUX_RX_ERR_CH1_REG",  HPU_HSSAER_AUX_RX_ERR_CH1_REG},
	{"HPU_HSSAER_AUX_RX_ERR_CH2_REG",  HPU_HSSAER_AUX_RX_ERR_CH2_REG},
	{"HPU_HSSAER_AUX_RX_ERR_CH3_REG",  HPU_HSSAER_AUX_RX_ERR_CH3_REG},
	{"HPU_SPNN_START_KEY_REG",         HPU_SPNN_START_KEY_REG},
	{"HPU_SPNN_STOP_KEY_REG",          HPU_SPNN_STOP_KEY_REG},
	{"HPU_SPNN_TX_MASK_REG",           HPU_SPNN_TX_MASK_REG},
	{"HPU_SPNN_RX_MASK_REG",           HPU_SPNN_RX_MASK_REG},
	{"HPU_TLASTTO_REG",                HPU_TLASTTO_REG},
	{"HPU_TLASTCNT_REG",               HPU_TLASTCNT_REG},
	{"HPU_TDATACNT_REG",               HPU_TDATACNT_REG}
};
...
static void hpu_flush_rx(struct hpu_priv *priv)
{
	...
	while (1) {
		...
		rx_IP_tlast_count = hpu_reg_read(priv, HPU_TLASTCNT_REG) >> 16;
		rx_IP_data_count = hpu_reg_read(priv, HPU_TDATACNT_REG) >> 16;
...
{
	priv->rx_suspended = 0;
	hpu_reg_write(priv, priv->rx_ctrl_reg, HPU_RX_CTRL_REG);
	hpu_reg_write(priv, priv->rx_aux_ctrl_reg, HPU_AUX_RX_CTRL_REG);
	...
	while (length > 0) {
		...
		while (1) {
			...
			case FIFO_STOPPED:
				...
				hpu_reg_write(priv, priv->irq_msk, HPU_MSK_REG);
...
static int hpu_spinn_set_keys(struct hpu_priv *priv, u32 start, u32 stop)
{
	...
	hpu_reg_write(priv, start, HPU_SPNN_START_KEY_REG);
	hpu_reg_write(priv, stop, HPU_SPNN_STOP_KEY_REG);
				 ...
				 int enable_l, int enable_r, int enable_aux)
{
	...
	hpu_reg_write(priv, priv->spinn_ctrl_reg, HPU_SPNN_CTRL_REG);
...
static void hpu_reset_tx_resync_timer(struct hpu_priv *priv)
{
	hpu_reg_write(priv, priv->tx_ctrl_reg |
		      HPU_TXCTRL_REG_FORCE_REARM, HPU_TX_CTRL_REG);
...
{
	hpu_reg_write(priv, priv->tx_ctrl_reg |
		      HPU_TXCTRL_REG_FORCE_RESYNC, HPU_TX_CTRL_REG);
...
static void _hpu_do_set_axis_lat(struct hpu_priv *priv)
{
	...
	hpu_reg_write(priv, lat, HPU_TLASTTO_REG);
}
...
static void hpu_set_aux_thrs(struct hpu_priv *priv, aux_cnt_t aux_cnt_reg)
{
	...
	reg = hpu_reg_read(priv, HPU_HSSAER_AUX_RX_ERR_THR_REG);
...
{
	u32 reg = hpu_reg_read(priv, HPU_STAT_RAW_REG);
...
static int hpu_chardev_open(struct inode *i, struct file *f)
{
	...
	/* default RX/TX mask */
	hpu_reg_write(priv, 0xFFFFFF, HPU_SPNN_TX_MASK_REG);
	hpu_reg_write(priv, 0xFFFFFF, HPU_SPNN_RX_MASK_REG);
	 ...
	 */
	priv->rx_tlast_count = hpu_reg_read(priv, HPU_TLASTCNT_REG) >> 16;
	priv->rx_data_count = hpu_reg_read(priv, HPU_TDATACNT_REG) >> 16;
		...
		for (i = 0; i < 4; i++) {
			if (num_channel & (1 << i))
				dev_info(&priv->pdev->dev,
					 "Aux CNT %d 0x%08X\n", i,
					 hpu_reg_read(priv,
					       (HPU_HSSAER_AUX_RX_ERR_CH0_REG + i * 4)));
		}
	}
}
...
static long hpu_ioctl(struct file *fp, unsigned int cmd, unsigned long _arg)
{
	...
	case _IOR(0x0, HPU_IOCTL_GET_AUX_THRS, unsigned int *):
		ret = hpu_reg_read(priv, HPU_HSSAER_AUX_RX_ERR_THR_REG);
		if (copy_to_user(arg, &ret, sizeof(unsigned int)))
			goto cfuser_err;
		break;

	case _IOR(0x0, HPU_IOCTL_GET_AUX_CNT0, unsigned int *):
		ret = hpu_reg_read(priv, HPU_HSSAER_AUX_RX_ERR_CH0_REG);
		if (copy_to_user(arg, &ret, sizeof(unsigned int)))
			goto cfuser_err;
		break;

	case _IOR(0x0, HPU_IOCTL_GET_AUX_CNT1, unsigned int *):
		ret = hpu_reg_read(priv, HPU_HSSAER_AUX_RX_ERR_CH1_REG);
		if (copy_to_user(arg, &ret, sizeof(unsigned int)))
			goto cfuser_err;
		break;

	case _IOR(0x0, HPU_IOCTL_GET_AUX_CNT2, unsigned int *):
		ret = hpu_reg_read(priv, HPU_HSSAER_AUX_RX_ERR_CH2_REG);
		if (copy_to_user(arg, &ret, sizeof(unsigned int)))
			goto cfuser_err;
		break;

	case _IOR(0x0, HPU_IOCTL_GET_AUX_CNT3, unsigned int *):
		ret = hpu_reg_read(priv, HPU_HSSAER_AUX_RX_ERR_CH3_REG);
		if (copy_to_user(arg, &ret, sizeof(unsigned int)))
			goto cfuser_err;
		break;
...
static int hpu_probe(struct platform_device *pdev)
{
	...
	hpu_clk_enable(priv);
	ver = hpu_reg_read(priv, HPU_ID_REG);
	hpu_clk_disable(priv);
	...
	tmp = hpu_reg_read(priv, HPU_IP_CNFG_REG);
